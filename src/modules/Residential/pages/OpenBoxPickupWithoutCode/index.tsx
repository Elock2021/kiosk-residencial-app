...
import BoxToOpenPickupWithoutCodeResidential from "../../../../components/BoxToOpenPickupWithoutCodeResidential";

const OpenBoxPickupWithoutCode = () => {
  ...
  const { pickup, order } = useSelector((state: any) => ({ pickup: state.pickup, order: state.order }));
  const { boxes } = pickup;
  ...
  const _handleConfirmModal = async () => {
    ...
    try {
      ...
      await Order.finishOrder({
        id: reservation.id,
        reservation_type: reservation?.reservation_type === 'residential_custody_delivered' ? 'residential_custody_picked' : "residential_picked",
        document: order.document,
      });
      dispatch(set_loader({ is_loading: true }));
      navigate("/pickup-without-code/packages?waitflow=true");
  ...
  return (
    <div className="container-fluid h-100">
      ...
      <div className="content-section px-4 d-flex flex-column justify-content-around">
        <BoxToOpenPickupWithoutCodeResidential action="pickup" />
...
export default OpenBoxPickupWithoutCode;
